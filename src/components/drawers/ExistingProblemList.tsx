  ...
  try {
    ...
    while (hasNext && requestCount < maxRequests) {
      ...
      const params: ProblemSearchParams = {
        ...
        difficulty: filterParams.difficulties.length === 1 ? filterParams.difficulties[0] : undefined,
        genre: filterParams.genres.length > 0 ? filterParams.genres.join(",") : undefined,
    ...
    <div className="w-1/2 border-r pr-4 overflow-y-auto space-y-4">
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <GenreFilter
          ...
          onGenreChange={(g) =>
            setFilters((prev) => ({ ...prev, genres: g }))
          }
        />
        <select
          value={filters.source}
          onChange={e => setFilters(prev => ({ ...prev, source: e.target.value as "CUSTOM" | "ORIGINAL" | "" }))}
        ...
        {(filters.searchQuery || filters.difficulties.length > 0 || filters.genres.length > 0 || filters.source) && (
          <Button
            variant="ghost"
            className="text-xs h-8"
            onClick={() =>
              setFilters({ searchQuery: "", difficulties: [], genres: [], source: "" })
            }
          >
            전체 해제
          </Button>

...
const GenreSelectionGroup = ({ 
  ...
  useEffect(() => {
    const filteredGenres = genres.filter(genre => genre !== "" && genre !== "none");
    const prevFiltered = prevGenresRef.current;
    
    // 배열 내용이 실제로 다를 때만 부모에게 알림
    const hasChanged = filteredGenres.length !== prevFiltered.length || 
  ...
  const allSelectedGenres = genres.filter(genre => genre !== "" && genre !== "none");
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium">
          장르 선택 {required && <span className="text-red-500">*</span>}
        </span>
        {required && !hasRequiredSelection && (
          <span className="text-xs text-red-500">(최소 1개 선택 필요)</span>
        )}
      </div>
      ...
      {/* 검증 상태 표시 */}
      {required && (
        <div className={`text-xs ${hasRequiredSelection ? 'text-gray-600' : 'text-red-500'}`}>
